...
// peersWithoutMultiSignBlock retrieves a list of peers that do not have a given multi signed block in
// their set of known hashes so it might be propagated to them.
func (ps *peerSet) peersWithoutMultiSignBlock(hash common.Hash) []*ethPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*ethPeer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if !p.KnownMultiSignBlock(hash) {
			list = append(list, p)
		}
	}
	return list
...
// peersWithoutMultiSignResult retrieves a list of peers that do not have a given multi signed result in
// their set of known hashes so it might be propagated to them.
func (ps *peerSet) peersWithoutMultiSignResult(hash common.Hash) []*ethPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*ethPeer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if !p.KnownMultiSignResult(hash) {
			list = append(list, p)
		}
	}
	return list
