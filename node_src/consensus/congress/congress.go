...
var (
	...
	// errInsufficientValidators is returned if the validator set is below the minimum required threshold.
...
// validateValidatorSet ensures the validator set meets security requirements
...
func (c *Congress) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	// Validate that the current validator set meets security requirements
	if err := c.validateValidatorSet(snap.validators()); err != nil {
		log.Error("Cannot verify seal: validator set validation failed", "error", err, "validators_count", len(snap.Validators))
		return err
	}
	...
	for seen, recent := range snap.Recents {
		if recent == signer {
			...
			if len(snap.Validators) < minValidators {
				return fmt.Errorf("%w: validator set size %d below minimum threshold %d", errInsufficientValidators, len(snap.Validators), minValidators)
			}
...
func (c *Congress) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	// Validate that the current validator set meets security requirements
	if err := c.validateValidatorSet(snap.validators()); err != nil {
		log.Error("Cannot prepare block: validator set validation failed", "error", err, "validators_count", len(snap.Validators))
		return err
	}
...
func (c *Congress) initializeSystemContracts(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	// Validate that the genesis validator set meets security requirements
	if err := c.validateValidatorSet(genesisValidators); err != nil {
		log.Error("Genesis validator set validation failed", "error", err, "validators_count", len(genesisValidators))
		return err
	}
...
func (c *Congress) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	...
	if err := c.validateValidatorSet(validators); err != nil {
		log.Error("Validator set validation failed", "error", err, "validators_count", len(validators))
		return []common.Address{}, err
	}
...
func (c *Congress) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// Validate that the validator set meets security requirements before updating
	if err := c.validateValidatorSet(vals); err != nil {
		log.Error("Cannot update validators: validation failed", "error", err, "validators_count", len(vals))
		return err
	}
...
func (c *Congress) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// Validate that the current validator set meets security requirements
	if err := c.validateValidatorSet(snap.validators()); err != nil {
		log.Error("Cannot seal block: validator set validation failed", "error", err, "validators_count", len(snap.Validators))
		return err
	}
